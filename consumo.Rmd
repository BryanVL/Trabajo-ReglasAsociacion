...
unique(consumo_ds$Hora)

consumo_ds %>% filter( ProductoComprado == "NONE"  )
...
```{r}
#Cambiamos la columna 'Hora' de manera que extraemos solamente la hora. Agrupamos por hora y calculamos el nº de ventas de cada uno de los productos por cada hora.
ventas_franja <- consumo_ds %>% 
          mutate( Hora = factor( format( strptime(Hora, "%H:%M:%S"), '%H') )) %>%
          group_by(Hora) %>%
          summarise("Num_ventas" = n())

#Visualizamos mediante otro gráfico de barras las ventas por franjas horarias.
g2 <- ggplot(ventas_franja, aes(x=Hora, y=Num_ventas)) +
...
consumo_ds2$año <- sapply(consumo_ds2$Fecha, getAño)
consumo_ds2$mes <- sapply(consumo_ds2$Fecha, getMes)
consumo_ds2$dia <- sapply(consumo_ds2$Fecha, getDia)
...
g3 <- ggplot(ventas_mes, aes(x=interaction(mes,año), y=Num_ventas)) +
      geom_bar(stat="identity", fill="lightblue", colour="black") +
      geom_text(aes(label=Num_ventas), vjust=-0.3, colour="black", size = 3.5) +
      theme_classic() + 
      ggtitle("Numero de ventas por mes") +
      labs(x = "Mes/Año", y="Ventas") +
      theme(axis.text.x = element_text(angle = 0, vjust = 0.5), 
      axis.title = element_text(size = 10, face = "bold") )
...
```{r}
prueba <- consumo_ds %>%
          group_by(IDComprador) %>%
          select(c(3,4))

lista.compra.usuarios <- split(prueba$ProductoComprado, prueba$IDComprador)
class(lista.compra.usuarios)
lista.compra.usuarios[1:10]
...
```{r}
reglas_s_redundantes <- reglas_s[is.redundant(reglas_s)]

reglas_s_noredundantes <- reglas_s[!is.redundant(reglas_s)]

( length(reglas_s_redundantes)/length(reglas_s) ) * 100
